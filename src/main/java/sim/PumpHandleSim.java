...
public class PumpHandleSim extends SimState {
	...
	public void start() {
		...
		// set up DISEASE
		//
		for(int j = 0; j < numInitialCases; j++) {
			Person initialCase = (Person) personGrid.allObjects.get(j); // random order
			Infection i = new Infection(initialCase, Infection.InfectionStatus.EXPOSED);
			schedule.scheduleOnce(i);
			
			if(indexCase == null) indexCase = i;
		}
